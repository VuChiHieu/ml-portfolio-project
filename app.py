...
def alert_box(name, text, kind="warning"):
    ...
    padding:12px 16px;display:flex;align-items:flex-start;margin:10px 0;font-size:14.5px;">
...
# ===== DASHBOARD 2: AI STOCK PREDICTION =====
    ...
    st.markdown("<br>", unsafe_allow_html=True)
    alert_box("alert-triangle", "Dự báo dựa trên OHLCV công khai, IC mô hình ~0.01-0.03 (yếu). Không dùng để đầu tư thực tế.", "warning")
...
# ===== DASHBOARD 3: RECOMMENDED PORTFOLIO =====
    ...
    with col2:
        ...
        st.plotly_chart(fig, use_container_width=True)
    alert_box("info", "Danh mục rebalance mỗi 5 ngày giao dịch theo đúng thiết kế backtest Phase 6-9.", "info")
...
# ===== DASHBOARD 4: BACKTEST =====
    ...
    alert_box("alert-triangle", "Kết luận: Mục tiêu Sharpe ≥ 1.8 KHÔNG đạt được. Chiến lược tốt nhất trên Test là Buy & Hold (Sharpe 1.068). Xem phân tích nguyên nhân trong báo cáo (Phase 12).", "error")
